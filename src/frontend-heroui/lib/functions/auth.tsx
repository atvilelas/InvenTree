...
import { api, setApiDefaults } from "../api";
    ...
    .catch((err) => {
      if (err?.response?.status == 401) {
        ...
        addToast({
          ...
          description: t`There is a conflicting session on the server for this browser. Please logout of that first.`,
          color: "danger",
        });
      }
    });
  ...
  if (user) {
    if (user.profile?.language && language != user.profile.language) {
      addToast({
        ...
        description: t`Your active language has been changed to the one set in your profile`,
        color: "primary",
        icon: "language",
      });
  ...
  api.post(apiUrl(ApiEndpoints.user_reset), values).then((val) => {
    if (val.status === 200) {
      addToast({
        ...
        description: t`Check your inbox for a reset link. This only works if you have an account. Check in spam too.`,
        color: "success",
      });
      navigate("/login");
    } else {
      addToast({
        ...
        description: t`Check your input and try again.`,
        color: "danger",
      });
    }
  });
...
export function handlePasswordReset(
  ...
  function success() {
    addToast({
      ...
      description: t`The password was set successfully. You can now login with your new password`,
      color: "success",
    });
    navigate("/login");
  }
  ...
  function passwordError(values: any) {
    addToast({
      title: t`Reset failed`,
      description: values?.errors.map((e: any) => e.message).join("\n"),
      color: "danger",
    });
  }
...
export function handleChangePassword(
  ...
  api
    ...
    .catch((err: HttpError | ResponseError) => {
      if ((err as HttpError).status === 401) {
        ...
        for (const val of (err as ResponseError).data.errors) {
          const typedVal = val as HttpError;

          if (typedVal.param && !errors[typedVal.param]) {
            errors[typedVal.param] = [];
          } else if (typedVal.param && errors[typedVal.param]) {
            errors[typedVal.param].push(val.message);
          }
        }
